...
func search(c *cli.Context) error {
    ...
    if keyword == "" {
		...
		resp, err := resty.New().R().SetQueryParam("query", keyword).Get(baseURL + "/book/search")
		if err != nil {
			return err
		}
    if err != nil {
        return cli.Exit(fmt.Sprintf("Failed to search books: %s", err), 1)
    }
    ...
    if err != nil {
        return cli.Exit(fmt.Sprintf("Failed to unmarshal books: %s", err), 1)
    }

    results := map[string][]Book{"books": result.Books}
...
func getBookInfo(id string)(info BookInfo,err error){
    ...
    if err != nil {
        return info,cli.Exit(fmt.Sprintf("Failed to get book info: %s", err), 1)
    }
    
    err = json.Unmarshal(resp.Body(), &info)
    if err != nil {
        return info, cli.Exit(fmt.Sprintf("Failed to unmarshal book info: %s", err), 1)
    }
    return
}
...
func content(c *cli.Context) error {
    ...
    if err != nil {
        return cli.Exit(fmt.Sprintf("Failed to unmarshal chapter content: %s", err), 1)
    }
...
func getChapterCount(bookID string) (r ChapterResp, err error) {
    ...
    if err != nil {
        return r,cli.Exit(fmt.Sprintf("Failed to get chapter list: %s", err), 1)
    }

    var chapters ChapterResp
    err = json.Unmarshal(resp.Body(), &chapters)
    if err != nil {
        return r,cli.Exit(fmt.Sprintf("Failed to unmarshal chapter list: %s", err), 1)
    }
    return chapters, nil
}

func getChapterContent(bookID string, chapterIndex int) (Content, error) {
	endpoint := fmt.Sprintf("%s/chapter/%s/%d", baseURL, bookID, chapterIndex)
    client := resty.New()
    resp, err := client.R().
        SetHeader("Content-Type", "application/json").
